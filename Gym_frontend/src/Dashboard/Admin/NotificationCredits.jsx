...
import { faCommentDots, faChartLine, faHistory, faShoppingCart, faCheckCircle } from '@fortawesome/free-solid-svg-icons';
  ...
  return (
    <div className="container-fluid p-3 p-md-4" style={{ backgroundColor: '#f8f9fa', minHeight: '100vh' }}>
      <div className="d-flex flex-column flex-sm-row justify-content-between align-items-sm-center mb-4 gap-3">
        <div>
          <h2 className="text-primary fw-bold mb-1" style={{ fontSize: 'clamp(1.2rem, 4vw, 1.6rem)' }}><FontAwesomeIcon icon={faCommentDots} className="me-2" /> Notification Credits</h2>
          <p className="text-muted mb-0">Manage your WhatsApp message credits and view usage history.</p>
        </div>
        <button className="btn btn-success fw-bold shadow-sm px-3 py-2 flex-shrink-0" onClick={() => setShowModal(true)}>
          <FontAwesomeIcon icon={faShoppingCart} className="me-2" /> Buy Credits
        </button>
      </div>
      ...
      <div className="row g-3 mb-4">
        <div className="col-6 col-md-4">
          <div className="card shadow-sm border-0 rounded-4 bg-white h-100">
            <div className="card-body p-3 d-flex align-items-center">
              <div className="rounded-circle bg-success bg-opacity-10 p-2 me-2 flex-shrink-0" style={{width:'44px',height:'44px',display:'flex',alignItems:'center',justifyContent:'center'}}>
                <FontAwesomeIcon icon={faCheckCircle} className="text-success" style={{fontSize:'1.1rem'}} />
              </div>
              <div className="overflow-hidden">
                <h6 className="text-muted mb-0 fw-bold" style={{fontSize:'clamp(10px,2.5vw,13px)'}}>Remaining Credits</h6>
                <h4 className="mb-0 fw-bold">{balance.remaining}</h4>
              </div>
            </div>
          </div>
        </div>
        <div className="col-6 col-md-4">
          <div className="card shadow-sm border-0 rounded-4 bg-white h-100">
            <div className="card-body p-3 d-flex align-items-center">
              <div className="rounded-circle bg-primary bg-opacity-10 p-2 me-2 flex-shrink-0" style={{width:'44px',height:'44px',display:'flex',alignItems:'center',justifyContent:'center'}}>
                <FontAwesomeIcon icon={faChartLine} className="text-primary" style={{fontSize:'1.1rem'}} />
              </div>
              <div className="overflow-hidden">
                <h6 className="text-muted mb-0 fw-bold" style={{fontSize:'clamp(10px,2.5vw,13px)'}}>Total Purchased</h6>
                <h4 className="mb-0 fw-bold">{balance.totalPurchased}</h4>
              </div>
            </div>
          </div>
        </div>
        <div className="col-6 col-md-4">
          <div className="card shadow-sm border-0 rounded-4 bg-white h-100">
            <div className="card-body p-3 d-flex align-items-center">
              <div className="rounded-circle bg-warning bg-opacity-10 p-2 me-2 flex-shrink-0" style={{width:'44px',height:'44px',display:'flex',alignItems:'center',justifyContent:'center'}}>
                <FontAwesomeIcon icon={faHistory} className="text-warning" style={{fontSize:'1.1rem'}} />
              </div>
              <div className="overflow-hidden">
                <h6 className="text-muted mb-0 fw-bold" style={{fontSize:'clamp(10px,2.5vw,13px)'}}>Credits Used</h6>
                <h4 className="mb-0 fw-bold">{balance.totalUsed}</h4>
              </div>
            </div>
          </div>
        </div>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} size="lg" centered>
        <Modal.Header closeButton>
          <Modal.Title className="fw-bold"><FontAwesomeIcon icon={faShoppingCart} className="me-2 text-success" /> Select a Credit Package</Modal.Title>
        </Modal.Header>
        <Modal.Body className="bg-light">
          <div className="row g-4">
            {packages.length === 0 ? (
              <div className="col-12 text-center py-4">No packages available.</div>
            ) : packages.map(pkg => (
              <div className="col-md-6" key={pkg.id}>
                <div className="card shadow-sm border-0 rounded-4 h-100 position-relative">
                  <div className="card-body p-4 text-center">
                    <h5 className="fw-bold text-primary mb-3">{pkg.packageName}</h5>
                    <div className="display-5 fw-bold mb-3">{pkg.credits} <span className="fs-5 text-muted fw-normal">Credits</span></div>
                    <h4 className="text-success fw-bold mb-4">₹{pkg.price}</h4>
