...
export const getVulnerableMembers = async (req, res, next) => {
  try {
    const branchId = req.user && req.user.role !== "SUPERADMIN" ? req.user.branchId : null;
    
    // Calculate attendance over the last 30 days
    let sql = `
      SELECT m.id, m.fullName, m.email, m.phone, m.branchId, m.profileImage,
             DATEDIFF(CURDATE(), (SELECT MAX(checkIn) FROM memberattendance WHERE memberId = m.id)) AS daysAbsent,
      ...
      ${branchId ? `AND m.branchId = ${branchId}` : ""}
      GROUP BY m.id
    `;
