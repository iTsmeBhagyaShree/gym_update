...
export const createPackage = async (req, res) => {
  try {
    const { packageName, credits, price } = req.body;
    ...
    const [result] = await pool.query(
      ...
      [packageName, credits, price, true]
    );
...
export const updatePackage = async (req, res) => {
  try {
    const { id } = req.params;
    const { packageName, credits, price, isActive } = req.body;
    
    await pool.query(
      `UPDATE credit_packages SET packageName=?, credits=?, price=?, isActive=? WHERE id=?`,
      [packageName, credits, price, isActive, id]
    );
...
export const getPackages = async (req, res) => {
  try {
    ...
    let query = `SELECT * FROM credit_packages ORDER BY credits ASC`;
    
    if (roleId !== 1 && roleId !== 9) {
      query = `SELECT * FROM credit_packages WHERE isActive = 1 ORDER BY credits ASC`;
    }
...
export const getCreditBalance = async (req, res) => {
  try {
    ...
    const [userRows] = await pool.query(`SELECT whatsappCredits, whatsappPlan FROM user WHERE id = ?`, [userId]);
    ...
    const [stats] = await pool.query(
      `SELECT SUM(creditsAdded) as totalPurchased, SUM(creditsUsed) as totalUsed 
       FROM whatsapp_credit_transactions WHERE userId = ?`,
      [userId]
    );
...
export const verifyPurchase = async (req, res) => {
  try {
    ...
    // Log transaction
    const description = 'Purchased ' + pkg.packageName + ' via Razorpay (' + razorpay_payment_id + ')';
    await pool.query(
      `INSERT INTO whatsapp_credit_transactions (userId, amountPaid, transactionType, description, creditsAdded, creditsUsed)
       VALUES (?, ?, 'PURCHASE', ?, ?, 0)`,
      [userId, pkg.price, description, pkg.credits]
    );
