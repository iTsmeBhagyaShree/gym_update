...
const Leads = () => {
  ...
  const handleBulkAllocate = async () => {
    const staffId = prompt("Enter Staff ID to allocate leads to:");
    const count = prompt("How many unassigned leads to allocate?");
    if (!staffId || !count) return;
    try {
      const user = JSON.parse(localStorage.getItem("user") || "{}");
      const adminId = user.adminId || user.id || localStorage.getItem("userId");
      const payload = { adminId, staffId, count: Number(count) };
  ...
  const handleConvertToMember = (lead) => {
    // Navigate to Walk-in registration with pre-filled lead details
  ...
  const exportToExcel = () => {
    ...
    const exportData = filteredLeads.map((m) => ({
      ...
      Source: m.source || "N/A",
      Status: m.status || "N/A",
      "Follow-up Date": m.followUpDate ? new Date(m.followUpDate).toLocaleDateString() : "N/A",
      Notes: m.notes || "N/A",
    }));
  ...
  const exportToPDF = () => {
    ...
    filteredLeads.forEach((m) => {
      const rowData = [
        ...
        m.source || "N/A",
        m.status || "N/A",
        m.followUpDate ? new Date(m.followUpDate).toLocaleDateString() : "N/A",
      ];
    ...
    <div className="container-fluid py-4">
      ...
      {(userRole === "SUPERADMIN" || userRole === "SUBADMIN") && crmStats && (
        <>
          ...
          {/* Charts & Top Staff Leaderboard Row */}
          <div className="row g-3 mb-4">
                ...
                <div className="card-body p-0" style={{ maxHeight: "320px", overflowY: "auto" }}>
                  <div className="table-responsive">
                    <table className="table table-hover align-middle mb-0">
                      ...
                      <tbody>
                        {crmStats.staffLeaderboard.length > 0 ? (
                          crmStats.staffLeaderboard.map((item, idx) => (
                            <tr key={idx}>
                              <td>
                                <div className="fw-semibold">{item.staffName}</div>
                                <small className="text-muted">{item.gymName} ({item.branchName})</small>
                              </td>
                              <td className="text-center fw-bold text-success">{item.convertedLeads}</td>
                              <td className="text-end">{item.conversionRate}%</td>
                            </tr>
                          ))
                        ) : (
                          <tr><td colSpan="3" className="text-center text-muted py-3">No data available</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
          ...
          {/* Gym Breakdown Analysis */}
          <div className="row g-3 mb-4">
            <div className="col-12">
              <div className="card border-0 shadow-sm" style={{ borderRadius: "15px" }}>
                <div className="card-header bg-white border-0 pt-4 fw-semibold" style={{ color: "#2B3674" }}>
                  Gym & Branch Performance Analysis
                </div>
                <div className="card-body">
                  <div className="table-responsive">
                    <table className="table table-hover align-middle mb-0">
                      ...
                      <tbody>
                        {crmStats.gymBreakdown.length > 0 ? (
                          crmStats.gymBreakdown.map((item, idx) => (
                            <tr key={idx}>
                              <td className="fw-semibold">{item.gymName}</td>
                              <td>{item.branchName}</td>
                              <td className="text-center">{item.totalLeads}</td>
                              <td className="text-center text-success fw-bold">{item.convertedLeads}</td>
                              <td className="text-end fw-bold">
                                <span className={`badge ${item.conversionRate >= 50 ? 'bg-success' : item.conversionRate >= 20 ? 'bg-warning' : 'bg-danger'}`}>
                                  {item.conversionRate}%
                                </span>
                              </td>
                            </tr>
                          ...
                          <tr><td colSpan="5" className="text-center text-muted py-3">No gyms registered</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
                    ...
                    <div className="row mb-3">
                      ...
                      <div className="col-md-6">
                        ...
                        <select className="form-select" name="assignedToStaffId" value={formData.assignedToStaffId || ""} onChange={handleChange} disabled={modalType === 'view'}>
                          ...
                          {staffList.map(staff => {
                            const roleName = staff.roleId === 4 ? "Trainer" : staff.roleId === 5 ? "Receptionist" : staff.roleId === 6 ? "Housekeeping" : staff.roleId === 3 ? "Manager" : "Staff";
                            return (
