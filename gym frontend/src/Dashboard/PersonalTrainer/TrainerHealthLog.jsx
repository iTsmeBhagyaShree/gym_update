  ...
  const fetchMembers = async () => {
    try {
      ...
      if (isPersonalTrainer()) {
        response = await axiosInstance.get(`members/trainer/${user.id}`);
      } else {
  ...
  const fetchLogs = async (memberId = "") => {
    try {
      ...
      if (memberId) {
        ...
        endpoint = `health/trainer/${user.id}`;
      } else {
      ...
      <div className="card shadow-sm border-0 mb-4" style={{ borderRadius: "15px" }}>
        <div className="card-header bg-white border-0 pt-4 pb-0 d-flex justify-content-between align-items-center">
          <div className="d-flex align-items-center w-50">
            <label className="form-label me-3 fw-bold text-nowrap mb-0" style={{ color: "#4318FF" }}>Select Member:</label>
            <select className="form-select w-75" value={selectedMember} onChange={handleMemberChange}>
